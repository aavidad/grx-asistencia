...
class Usuarios(Gtk.Grid):
	...
	Btn_Pant = builder.get_object("Btn_Pant")
	Btn_Pant.connect("clicked", self.on_Btn_Pant_clicked,usuario,nombre,ip,puerto,password)
...
"""
    ...
    def on_Btn_Pant_clicked(self, widget, usuario,nombre,ip ,puerto,password):
      with settings(host_string=ip,port=puerto,password=password,user=usuario):
		try:
			os.system('ssh -Y -p'+puerto+' '+usuario+'@'+ip+' "DISPLAY=:0.0 import -window root -format png - | display -format png -" &') 
		except:
...
################################################
	...
	with settings(host_string=ip,port=puerto,password=password,user=usuario):
		try:
			...
			self.mensaje ("No se ha podido montar la carpeta remota","ATENCION Se ha producido un ERROR:",error)
        spinner.stop()
        estado.set_text("")
