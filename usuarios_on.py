...
class Usuarios(Gtk.Grid):
    def __init__(self,widget,nombre,display,ip,usuario,puerto,password,tecnico,password_tecnico,clave_cifrado):
    ...
    def on_entry_veleta2_icon_press(self, widget,enum,void, ip, usuario,nombre, puerto,password):
...
"""
			...
			sudo ('chown '+nombre+'."usuarios del dominio" /home/'+nombre+'/.pam_mount.conf.xml')
    ...
    def on_entry_mensaje_icon_press(self, widget,enum,void,ip,puerto,usuario,password,nombre,display):
	try:
		mensa=widget.get_text()
		if mensa=="":
			return True
		with settings(host_string=ip,port=puerto,user=usuario,password=password):
			sudo (' DISPLAY='+display+' XAUTHORITY=/home/'+nombre+'/.Xauthority notify-send -t 5000 "'+mensa+'" ;sleep 1 ')
			os.system('notify-send -t 2 "Mensaje: '+mensa+' enviado a '+nombre+'"')
		return True
	except:
		self.mensaje ("No ha sido posible mandar el mensaje","Atencion",atencion)


    def on_Btn_sonido_clicked(self, widget,nombre, ip,usuario,puerto,password,display):
 	with settings(host_string=ip,port=puerto,password=password,user=usuario):
		try:	
			put (remote_path="/tmp/sonido.avi",local_path="/tmp/sonido.avi")
			sudo (' DISPLAY='+display+' XAUTHORITY=/home/'+nombre+'/.Xauthority xdg-open /tmp/sonido.avi &')
			
		except:
			self.mensaje ("No se puede reproducir el sonido en el equipo remoto","ATENCION Se ha producido un ERROR:",error)

    def on_Btn_usado_clicked(self, widget,nombre, ip,usuario,puerto,password,display):
 	with settings(host_string=ip,port=puerto,password=password,user=usuario):
		try:	
    ...
    def on_Btn_video_clicked(self, widget,nombre, ip,usuario,puerto,password,notebook):
    	print "nada"



    def on_Btn_color_usuario_color_set(self, widget):
	color=widget.get_color()
	rgba = Gdk.RGBA.from_color(color)
	Box_usuarios.override_background_color(0, rgba)
	self.show_all()
...
#					entry_veleta.set_text(ruta)
			...
			os.system(comando)
			resultado=os.open("find . -maxdepth 2 -type d -iname "+nombre)
			if resultado=="":
				print "no encontrado"
				print resultado
			else:
		...
		tab_label = tablabel.TabLabel("ldap "+ nombre,Gtk.Image.new_from_file("./icons/info_equipo.png"))
		tab_label.connect("close-clicked", tablabel.on_close_clicked, notebook, scrolledwindow)
		notebook.append_page(scrolledwindow ,tab_label)
		self.show_all()
################################################
