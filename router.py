...
class Router(Gtk.Grid):
    def __init__(self,ip):
        Gtk.Grid.__init__(self, row_spacing=20, column_spacing=20)
        builder = Gtk.Builder()
        builder.add_from_file("/usr/share/grx/glade/router.glade")
	box_router = builder.get_object("box_router")
	notebook = builder.get_object("notebook_router")

	Btn_web = builder.get_object("Btn_web")
	Btn_web.connect("clicked", self.on_Btn_web_clicked,ip)

	Btn_router = builder.get_object("Btn_router")
	Btn_router.connect("clicked", self.on_Btn_router_clicked,ip,notebook)
    ...
    def on_Btn_router_clicked(self, widget, ip,notebook):
	...
	scrolledwindow.add(page)
		...
		self.mensaje('No se ha podido abrir la web del router')	
